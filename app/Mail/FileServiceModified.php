...
class FileServiceModified extends Mailable
{
    ...
    {
        $emailTemplate = EmailTemplate::where('company_id', $this->fileService->user->company->id)->where('label', 'file-service-modified-email')->first(['subject', 'body']);
        if($emailTemplate){
            $subject = $emailTemplate->subject;
			$subject = str_replace('##CUSTOMER_NAME', $this->fileService->user->full_name, $subject);
