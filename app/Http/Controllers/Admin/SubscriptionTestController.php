 ...
 */
class SubscriptionTestController extends MasterController
{
    ...
    public function executeSubscription(Request $request){
        if ($request->has('success') && $request->query('success') == 'true') {
            ...
            try {
                ...
                $subscription->description = 'Amount: '.$currencySymbol.round($subscription->shipping_amount->value);
                $subscription->start_date = \Carbon\Carbon::parse($subscription->start_time)->format('Y-m-d H:i:s');
